...
public class Arithmetic_copy {
    ...
    static boolean findAnswer(int pos, int value) {
        ...
        op = '^';
        if (findAnswer(pos + 1, (int) Math.round(Math.pow(value,nums[pos])))) {
            ops[pos - 1] = op;
            return true;
        }
    ...
    // }
    // char op = '*';
    // if (findAnswer(pos + 1, value + (timesNum * order + value * (1 - order)) *
    // (nums[pos] - 1),
    // timesNum * nums[pos])) {
    // ops[pos - 1] = op;
    // return true;
