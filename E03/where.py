...
def process_tokens():
    ...
    if (re.fullmatch(r'[+-]?[0-9]+\.?[0-9]*', tokens[0])
            and re.fullmatch(r'[+-]?[0-9]+\.?[0-9]*', tokens[1])):
        standard_form(float(tokens.pop(0)), float(tokens.pop(0)))
        return
    # format is num hem num hem (standard form with hemisphere with spaces)
    if (re.fullmatch(r'[0-9]+\.?[0-9]*', tokens[0])
            and re.fullmatch(r'[0-9]+\.?[0-9]*', tokens[2])
            and re.fullmatch(r'[NSEW]', tokens[1])
            and re.fullmatch(r'[NSEW]', tokens[3])):
    ...
    if (re.fullmatch(r'[0-9]+\.?[0-9]*?[NSEW]', tokens[0])
            and re.fullmatch(r'[0-9]+\.?[0-9]*?[NSEW]', tokens[1])):
    ...
    # if re.fullmatch(r'[0-9]+', tokens[0])
